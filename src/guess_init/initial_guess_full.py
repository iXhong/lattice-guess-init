...
def estimate_one_state_initial_guess(
    samples_boot_time: np.ndarray,
    ...
    """Estimate one-state parameters for all candidate intervals and refine with lmfit."""
    samples = np.asarray(samples_boot_time, dtype=float)
    if samples.ndim != 2:
    ...
    print(f"Found {len(ground_intervals)} candidate ground-state intervals from t={start_time_index} to t={end_time_ground}")
    print(f"Candidate intervals: {[ (int(inv[0]), int(inv[-1])) for inv in ground_intervals ]}")
    ...
    for idx, candidate in enumerate(ground_linear["candidates"]):
        # refine with lmfit for each candidate interval
        try:
            refined = refine_ground_with_lmfit(
                corr_mean,
                corr_err,
                time_slices,
                candidate["interval"],
            ...
            result_interval.append(
                {
                    "interval_start": int(candidate["interval"][0]),
                    "interval_end": int(candidate["interval"][-1]),
                    "log_amp": refined["log_amp"],
                    "mass": refined["mass"],
                    "fit_success": refined["fit_success"],
                    "chisqr": refined["chisqr"],
                    "redchi": refined["redchi"],
                }
            )
        except Exception as e:
            print(f"  Refinement failed for Candidate {idx} with error: {e}")
...
def estimate_two_state_initial_guess(
    ...
    for g_inv in ground_intervals:
        ...
        print(f"Interval [{t_start}, {int(g_inv[-1])}]: Local ground state refined to A1={local_A1:.3f}, m1={local_m1:.3f}")
    ...
    return {
        ...
        "median_ground_interval": median_ground_inv,
        "all_intervals_results": all_intervals_results,
    }


if __name__ == "__main__":
    # 这里可以添加一些简单的测试代码，或者留空
    t = np.arange(49)
    data = np.load("data/processed/bs/p2_0_bs.npy")

    result_one = estimate_one_state_initial_guess(
    ...
    print(f"Estimated one-state initial guess: {result_one}")
    ...
    #     sigma_band=5.0,
    # )
    # print(f"Estimated two-state initial guess: {result}")
